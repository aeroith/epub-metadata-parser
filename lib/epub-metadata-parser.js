            ...
            zipfile.on("entry", function (entry) {
              ...
              zipfile
                .openReadStream(entry, function (err, readStream) {
                  ...
                  if (fileName === '') 
                    zipfile.readEntry() // xml file is on the top directory level // if valid continue; // get the xml parse and try to find cover image if exists it is assumed that
              ...
              yauzl.open(path, {
                ...
                zipfile.on("entry", function (entry) {
                  ...
                  zipfile.openReadStream(entry, function (err, readStream) {
                    ...
                    else if ((fileName.split(".").pop() === "png" || fileName.split(".").pop() === "jpg" || fileName.split(".").pop() === "jpeg" )
                     && (fileName.split(".")[0] === "cover" || fileName.split(".")[0] === "kapak")) {
                      bookData.cover = fileName;
                      readStream.pipe(fs.createWriteStream("./" + bookData.tmp + fileName));
                      readStream.on("end", function () {
                        zipfile.readEntry();
